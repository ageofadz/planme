...
function fillScreen(tl: {term: string, image: string}[]) {
    const contents = []
...
export default function Presentation(props: {rows: Row[], options: Options}) {
    ...
    const tl = JSON.parse(tlString!) as {term: string, image: string}[]
    ...
    for (const row of rows) {
        ...
        switch (row.name as Activity) {
            ...
            case Activity.StickyTargets:{
                ...
                for (const tlItem of tl) {
                var terms = [(tl.at(Math.floor(tl.length*Math.random()))?.term),(tl.at(Math.floor(tl.length*Math.random()))?.term),tlItem.term]
                terms = terms
